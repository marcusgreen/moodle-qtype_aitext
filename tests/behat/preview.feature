...
Feature: Preview aitext questions
  ...
  @javascript @_switch_window
  Scenario: Preview an aitext question that uses the HTML editor with embedded files.
  ...
  Scenario: Preview an aitext question that uses a plain text area.
    When I am on the "aitext-003" "core_question > preview" page logged in as teacher
    And I expand all fieldsets
    And I set the field "How questions behave" to "Immediate feedback"
    And I press "Start again with these options"
    And I should see "Please write a story about a frog."
